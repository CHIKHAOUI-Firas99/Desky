...
@Component({
  ...
  styleUrls: ['./users-reservations.component.scss']
})
export class UsersReservationsComponent {
  groups: any[] = [];
  ...
  displayedColumns: string[] = ["date", "start_time", "end_time","user_id","username","desk_id","workspace","type","status"];
    ...
    if (this.reservations.filter.length > 0) {
      let u = this.tabUserReservations.filter(
        (n) =>
          ...
          n.name.toString().toLowerCase().includes(this.reservations.filter)||
          n.user_id.toString().toLowerCase().includes(this.reservations.filter)
        ...
        tab
          ...
          .map( (val) => {
            ...
            return  this.fb.group({
              ...
              username:new FormControl(val.name),
...
getPictureUrl(picture) {
  ...
  async getDemands() {
    try {
      ...
      const data = await this._reservationserviceService.get_all_reservations().toPromise();
      console.log(data);

...
import { AbstractControl, ControlValueAccessor, FormArray, FormBuilder, FormControl, FormGroup } from '@angular/forms';
...
displayPassword=false
  ...
  applyFilter(event: Event) 
  {
      ...
      {
          let u=    this.TabRoles.filter((n)=> n.id.toString().includes(this.roles.filter)|| 
         
              n.id.includes(this.roles.filter) || n.name.includes(this.roles.filter) 
   ...
   ngOnInit() 
  {
    this._authService.dispalyAdminDashboard().subscribe((data) => {
      ...
      this._roleService.getAll().subscribe(data=>{this.tabRoleNames=data;})
...
modelgrp=[]
    fillFormTab(tab: Array<Role>) {
      this.VOForm = this.fb.group({
        VORows: this.fb.array(tab.filter(user => user != undefined).map(val => {        
  ...
  return this.fb.group({
    ...
    console.log(this.modelgrp);
      ...
      this.roles = new MatTableDataSource((this.VOForm.get('VORows') as FormArray).controls);
    }
...
return objects
    ...
    SaveVO(VOFormElement, i) {
      ...
      validatedClaims.forEach(element => {
        let obj = '';
        const t = this.allclaims.find((n) => n['items'].find(item => item['value'] === element));
        console.log(t);
    ...
    let  obb=this.getObjectNames(tabValidatedClaims)
   console.log(this.evthgrp[i])
console.log(tabValidatedClaims);
  ...
  console.log(tab);
    ...
    CancelSVO(VOFormElement, i) {
      ...
      row.get('claims').patchValue(this._RoleService. transformObject(this.TabRoles[i].claims)[0]);
